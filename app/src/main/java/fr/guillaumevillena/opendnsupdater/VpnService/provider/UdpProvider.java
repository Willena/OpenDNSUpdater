...
public class UdpProvider extends Provider {
    ...
    public UdpProvider(ParcelFileDescriptor descriptor, OpenDnsVpnService service) {
    ...
    protected void forwardPacket(DatagramPacket outPacket, IpPacket parsedPacket) {
        ...
        try {
            ...
            Log.w(TAG, "DNSProvider: Could not send packet to upstream, forwarding packet directly");
        }
    }
    ...
    private void handleRawDnsResponse(IpPacket parsedPacket, DatagramSocket dnsSocket) {
        try {
            ...
            Log.e(TAG, "handleRawDnsResponse: ", e);
        }
    }
     ...
     */
    @Override
    protected void handleDnsRequest(byte[] packetData) throws VpnNetworkException {
        ...
        try {
            ...
            Log.e(TAG, "handleDnsRequest: DNS server alias query failed for " + destAddr.getHostAddress(), e);
            return;
        }
        ...
        try {
            ...
            if (OpenDnsUpdater.getPrefs().getBoolean("settings_debug_output", false)) {
                Log.d(TAG, "DnsRequest: " + dnsMsg.toString());
            }
        } catch (IOException e) {
